...
import { UserDetailsQuery, MetaQuery, AuditLogsQuery } from '../graphql/queries';
import type {
  UsersResponse,
  MetaResponse,
  AuditLogsResponse,
    ...
    const fetchData = async () => {
      ...
      try {
        const [usersRes, metaRes, auditRes] = await Promise.all([
          client.query<UsersResponse>(UserDetailsQuery, { params: { limit: 1, page: 1 } }).toPromise(),
          client.query<MetaResponse>(MetaQuery, {}).toPromise(),
          client.query<AuditLogsResponse>(AuditLogsQuery, { params: { limit: 5, page: 1 } }).toPromise(),
        ]);
