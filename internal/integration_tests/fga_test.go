...
func initFGATestSetup(t *testing.T, cfg *config.Config) (*testSetup, engine.AuthorizationEngine) {
	...
	require.NoError(t, err)
	authProvider, err := authenticators.New(cfg, &authenticators.Dependencies{Log: &logger, StorageProvider: storageProvider, MemoryStoreProvider: memoryStoreProvider})
	require.NoError(t, err)
