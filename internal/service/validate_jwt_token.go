...
// refresh_token
// Permission: none
func (s *service) ValidateJWTToken(ctx context.Context, params *model.ValidateJWTTokenInput) (*model.ValidateJWTTokenResponse, error) {
	log := s.Log.With().Str("func", "ValidateJWTToken").Logger()
	...
	claims, err = s.TokenProvider.ParseJWTToken(params.Token)
	if err != nil {
	...
	if tokenType == constants.TokenTypeAccessToken || tokenType == constants.TokenTypeRefreshToken {
		...
		token, err := s.MemoryStoreProvider.GetUserSession(sessionKey, tokenType+"_"+claims["nonce"].(string))
		if err != nil || token == "" {
	...
	if nonce != "" {
		if ok, err := s.TokenProvider.ValidateJWTClaims(claims, &token.AuthTokenConfig{
			HostName: hostname,
			Nonce:    nonce,
			User: &schemas.User{
				ID: userID,
		...
		if ok, err := s.TokenProvider.ValidateJWTTokenWithoutNonce(claims, &token.AuthTokenConfig{
			HostName: hostname,
			User: &schemas.User{
				ID: userID,
			},
	...
	claimKey := s.Config.JWTRoleClaim
