...
// Permissions: none
func (s *service) ResetPassword(ctx context.Context, params *model.ResetPasswordInput) (*model.Response, error) {
	log := s.Log.With().Str("func", "ResetPassword").Logger()
	...
	if isTokenVerification {
		verificationRequest, err = s.StorageProvider.GetVerificationRequestByToken(ctx, verifyingToken)
		if err != nil {
		...
		claim, err := s.TokenProvider.ParseJWTToken(verifyingToken)
		if err != nil {
	...
	if isOtpVerification {
		...
		otpRequest, err = s.StorageProvider.GetOTPByPhoneNumber(ctx, phoneNumber)
		if err != nil {
	...
	if isOtpVerification {
		...
		err = s.StorageProvider.DeleteOTP(ctx, otpRequest)
		if err != nil {
