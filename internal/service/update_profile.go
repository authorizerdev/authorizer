...
func (s *service) UpdateProfile(ctx context.Context, params *model.UpdateProfileInput) (*model.Response, error) {
	log := s.Log.With().Str("func", "UpdateProfile").Logger()
	...
	if params.NewPassword != nil && params.ConfirmNewPassword != nil {
		...
		if err := validators.IsValidPassword(refs.StringValue(params.NewPassword), s.Config.DisableStrongPassword); err != nil {
	...
	if params.Email != nil && refs.StringValue(user.Email) != refs.StringValue(params.Email) {
		...
		if !isEmailVerificationDisabled {
			...
			go s.EmailProvider.SendEmail([]string{refs.StringValue(user.Email)}, verificationType, map[string]interface{}{
				"user":             user.ToMap(),
	...
	_, err = s.StorageProvider.UpdateUser(ctx, user)
	if err != nil {
