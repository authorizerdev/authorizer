...
// Permissions: none
func (s *service) ResendOTP(ctx context.Context, params *model.ResendOTPRequest) (*model.Response, error) {
	...
	log := s.Log.With().Str("func", "ResendOTP").Str("email", email).Str("phone_number", phoneNumber).Logger()
	...
	if email != "" {
		otpData, err = s.StorageProvider.GetOTPByEmail(ctx, refs.StringValue(params.Email))
		log.Debug().Msg("Failed to get otp for given email")
	} else {
		otpData, err = s.StorageProvider.GetOTPByPhoneNumber(ctx, refs.StringValue(params.PhoneNumber))
