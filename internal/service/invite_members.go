...
func (s *service) InviteMembers(ctx context.Context, params *model.InviteMemberInput) (*model.InviteMembersResponse, error) {
	log := s.Log.With().Str("func", "InviteMembers").Logger()
	...
	isBasicAuthDisabled := s.DisableBasicAuthentication
	isMagicLinkLoginDisabled := s.DisableMagicLinkLogin
	...
	for _, email := range emails {
		_, err := s.StorageProvider.GetUserByEmail(ctx, email)
		if err != nil {
	...
	for _, email := range newEmails {
		defaultRoles := strings.Split(s.Config.DefaultRoles, ",")
		...
		_, err = s.StorageProvider.AddVerificationRequest(ctx, verificationRequest)
		if err != nil {
		...
		go s.EmailProvider.SendEmail([]string{refs.StringValue(user.Email)}, constants.VerificationTypeInviteMember, map[string]interface{}{
			"user":             user.ToMap(),
