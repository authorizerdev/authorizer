...
// Permissions: none
func (s *service) ResendVerifyEmail(ctx context.Context, params *model.ResendVerifyEmailInput) (*model.Response, error) {
	log := s.Log.With().Str("func", "ResendVerifyEmail").Logger()
	...
	verificationRequest, err := s.StorageProvider.GetVerificationRequestByEmail(ctx, params.Email, params.Identifier)
	if err != nil {
	...
	go s.EmailProvider.SendEmail([]string{params.Email}, params.Identifier, map[string]interface{}{
		"user":             user.ToMap(),
