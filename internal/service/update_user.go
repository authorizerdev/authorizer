...
func (s *service) UpdateUser(ctx context.Context, params *model.UpdateUserInput) (*model.User, error) {
	log := s.Log.With().Str("func", "UpdateUser").Logger()
	...
	user, err := s.StorageProvider.GetUserByID(ctx, params.ID)
	if err != nil {
	...
	// 	if _, err := s.StorageProvider.GetUserByPhoneNumber(ctx, strings.TrimSpace(refs.StringValue(params.PhoneNumber))); err == nil {
	...
	if params.IsMultiFactorAuthEnabled != nil && refs.BoolValue(user.IsMultiFactorAuthEnabled) != refs.BoolValue(params.IsMultiFactorAuthEnabled) {
		...
		if refs.BoolValue(params.IsMultiFactorAuthEnabled) {
			...
			isMailOTPEnvServiceDisabled := s.Config.DisableEmailOTP
			isTOTPEnvServiceDisabled := s.Config.DisableTOTPLogin
			isSMSOTPEnvServiceDisabled := s.Config.DisableSMSOTP
	...
	if params.Email != nil && refs.StringValue(user.Email) != refs.StringValue(params.Email) {
		...
		_, err = s.StorageProvider.GetUserByEmail(ctx, newEmail)
		...
		go s.MemoryStoreProvider.DeleteAllUserSessions(user.ID)
		...
		go s.EmailProvider.SendEmail([]string{refs.StringValue(user.Email)}, constants.VerificationTypeBasicAuthSignup, map[string]interface{}{
			"user":             user.ToMap(),
	...
	if params.PhoneNumber != nil && refs.StringValue(user.PhoneNumber) != refs.StringValue(params.PhoneNumber) {
		...
		_, err = s.StorageProvider.GetUserByPhoneNumber(ctx, phone)
		// err = nil means user exists
		if err == nil {
	...
	if len(params.Roles) > 0 {
		...
		protectedRoles := strings.Split(s.Config.ProtectedRoles, ",")
